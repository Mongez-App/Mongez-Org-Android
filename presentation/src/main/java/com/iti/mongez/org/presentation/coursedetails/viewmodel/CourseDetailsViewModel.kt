package com.iti.mongez.org.presentation.coursedetails.viewmodel
...
import com.iti.mongez.org.domain.courses.usecase.*
    ...
    private fun loadCourseData() {
        ...
        viewModelScope.launch {
            ...
            when (val detailsResult = getCourseDetailsUseCase(courseId)) {
                is Result.Success<*> -> {
                    val course = detailsResult.data as Course
            ...
            when (val materialsResult = getCourseMaterialsUseCase(courseId)) {
                is Result.Success<*> -> {
                    val documents = (materialsResult.data as List<CourseMaterial>).map { mat ->
            ...
            when (val tasksResult = getCourseTasksUseCase(courseId)) {
                is Result.Success<*> -> {
                    allTasks = (tasksResult.data as List<CourseTask>).map { task ->
                        TaskItem(
                            id = task.id,
                            title = task.title,
